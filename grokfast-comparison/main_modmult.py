        ...
        for data, is_train in [(train_data, True), (valid_data, False)]:
            ...
            for input in dl:
                ...
                if is_train:       
                    ...
                    epsilon = 1e-9
                    if args.mode == "egd":
                        if args.rank_svd is not None:
                            rank = args.rank_svd
                        G2 = model.layers[0].mlp[0].weight.grad.detach().clone()
                        if args.rank_svd is not None:
                            U2, S2, Vh2 = randomized_svd_torch(G2, rank=rank)
                        else:
                            U2, S2, Vh2 = torch.linalg.svd(G2, full_matrices=False)
                        S_inv2 = 1.0 / torch.clamp(S2, min=epsilon)
                        G2_precond = (U2 * S_inv2.unsqueeze(0)) @ (U2.T @ G2)
                        model.layers[0].mlp[0].weight.grad.copy_(G2_precond)

                        G2 = model.layers[0].mlp[2].weight.grad.detach().clone()
                        if args.rank_svd is not None:
                            U2, S2, Vh2 = randomized_svd_torch(G2, rank=rank)
                        else:
                            U2, S2, Vh2 = torch.linalg.svd(G2, full_matrices=False)
                        S_inv2 = 1.0 / torch.clamp(S2, min=epsilon)
                        G2_precond = (U2 * S_inv2.unsqueeze(0)) @ (U2.T @ G2)
                        model.layers[0].mlp[2].weight.grad.copy_(G2_precond)


                        G2 = model.layers[1].mlp[0].weight.grad.detach().clone()
                        if args.rank_svd is not None:
                            U2, S2, Vh2 = randomized_svd_torch(G2, rank=rank)
                        else:
                            U2, S2, Vh2 = torch.linalg.svd(G2, full_matrices=False)
                        S_inv2 = 1.0 / torch.clamp(S2, min=epsilon)
                        G2_precond = (U2 * S_inv2.unsqueeze(0)) @ (U2.T @ G2)
                        model.layers[1].mlp[0].weight.grad.copy_(G2_precond)

                        G2 = model.layers[1].mlp[2].weight.grad.detach().clone()
                        if args.rank_svd is not None:
                            U2, S2, Vh2 = randomized_svd_torch(G2, rank=rank)
                        else:
                            U2, S2, Vh2 = torch.linalg.svd(G2, full_matrices=False)
                        S_inv2 = 1.0 / torch.clamp(S2, min=epsilon)
                        G2_precond = (U2 * S_inv2.unsqueeze(0)) @ (U2.T @ G2)
                        model.layers[1].mlp[2].weight.grad.copy_(G2_precond)
                    elif args.mode == "grokfast":
                        pass
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", default="grokfast")
