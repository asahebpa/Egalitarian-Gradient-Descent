...
def randomized_svd_torch(G, rank, n_iter=2):
    m, n = G.shape
    Q = torch.randn(n, rank, device=G.device, dtype=G.dtype)
    for _ in range(n_iter):
        Q = torch.linalg.qr(G @ Q, mode='reduced')[0]
        Q = torch.linalg.qr(G.T @ Q, mode='reduced')[0]
    B = G @ Q
...
def main(args):
    log_freq = math.ceil(args.optimization_steps / 2)
    ...
    with tqdm(total=args.optimization_steps, dynamic_ncols=True) as pbar:
        for x, labels in islice(cycle(train_loader), args.optimization_steps):
            ...
            if do_log:
                ...
                # track when test acc >= 0.85
                if epoch_reached is None and test_accuracies[-1] >= 0.85:
                    epoch_reached = steps
            ...
            optimizer.zero_grad()
            loss.backward()
            if len(layers_to_egd) > 0:
                epsilon = 1e-6
                for l in layers_to_egd:
                    G = mlp[l].weight.grad.detach().clone()
                    if args.rank_svd != None:
                        # Randomized SVD approximation
                        U, S, Vh = randomized_svd_torch(G, rank=args.rank_svd)
                    else:
                        U, S, Vh = torch.linalg.svd(G, full_matrices=False)

                    # Inverse of singular values (preconditioner)
                    S_inv = 1.0 / torch.clamp(torch.tensor(S, device=G.device, dtype=G.dtype), min=epsilon)
                    
                    # Project gradient using approximate inverse
                    U = torch.tensor(U, device=G.device, dtype=G.dtype)
                    Vh = torch.tensor(Vh, device=G.device, dtype=G.dtype)
                    G_precond = (U * S_inv.unsqueeze(0)) @ (U.T @ G)

                    mlp[l].weight.grad.copy_(G_precond)
    ...
    print(f"\nTotal wall-clock runtime: {wallclock/60:.2f} minutes")

    if epoch_reached is not None:
        print(f"Reached 85% test accuracy at step {epoch_reached}")
    else:
        print("Test accuracy never reached 85%")       
...
if __name__ == '__main__':
    ...
    parser.add_argument("--rank_svd", type=int, default=None)
